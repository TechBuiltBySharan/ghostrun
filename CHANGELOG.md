...
- **`doctor` never checked whether the Playwright Chromium browser binary was actually installed** — only `runInit` (the setup wizard) verified this; `doctor` had no Chromium check at all, so `run` could fail on its first real flow (`Executable doesn't exist...`) with no earlier warning from the one command whose entire job is to catch exactly this. Added a `Playwright Chromium browser` check to `doctor` using `chromium.executablePath()` + `fs.existsSync()` — fast and precise, no subprocess needed.
- **A leftover `ghostrun flow:schedule <id> "<cron>"` suggestion** in the empty-schedule-list message pointed at a command that's been rejected as legacy since alpha.12 — missed in that pass. Now suggests `ghostrun monitor schedule add <id> "<cron>"`.
